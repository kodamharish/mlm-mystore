...
def format_address_block(addr):
    ...
    lines.append(f"{addr.city}, {addr.state} - {addr.pincode}")
    lines.append(addr.country or "India")
...
def generate_product_invoice_pdf(transaction, order, user):
    ...
    # ================= PRODUCT TABLE =================
    col_widths = [25, 200, 40, 55, 35, 40, 40, 40, 40, 55]
    ...
    for idx, item in enumerate(order.items.all(), start=1):
        ...
        # For now GST split shown as 0 (customize if you have tax logic)
        product_data.append([
            str(idx),
            desc,
            "0",
